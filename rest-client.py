...
def doDotProduct(addr, debug=False):
    headers = {'content-type': 'application/json'}
    vector1 = [random.randint(1, 10) for _ in range(10)] 
    vector2 = [random.randint(1, 10) for _ in range(10)]  
    data = {
        'vector1': vector1,
        'vector2': vector2
    }
    # send http request with JSON data and receive response
    dotproduct_url = addr + "/api/dotproduct"
    response = requests.post(dotproduct_url, json=data, headers=headers)
    if debug:
        # decode response
        print("Response is", response)
        print(json.loads(response.text))
